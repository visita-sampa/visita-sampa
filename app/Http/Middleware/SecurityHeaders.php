...
class SecurityHeaders
{
  ...
  public function handle($request, Closure $next)
  {
    ...
    if (!app()->environment('testing')) {
      ...
      $response->headers->set('Content-Security-Policy', "default-src 'self'; script-src 'self' platform.twitter.com plausible.io utteranc.es *.cloudflare.com *.jsdelivr.net 'unsafe-inline' 'unsafe-eval' plausible.io/js/plausible.js utteranc.es/client.js; style-src 'self' *.cloudflare.com *.jsdelivr.net *.googleapis.com 'unsafe-inline'; img-src 'self' * data:; font-src 'self' *.googleapis.com ; connect-src 'self' plausible.io/api/event; media-src 'self'; frame-src 'self' platform.twitter.com plausible.io utteranc.es github.com *.youtube.com *.vimeo.com; object-src 'none'; base-uri 'self';");
